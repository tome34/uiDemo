...
import com.fec.view.common.utils.LangHelper;
import com.fec.view.common.utils.ResHelper;
...
public class TopBar extends RelativeLayout {
    ...
    // 这个构造器只用于内部，不开放给外面用，目前用于TopBarLayout
    TopBar(Context context, boolean inTopBarLayout) {
        super(context);
        initVar();
        if (inTopBarLayout) {
            //int transparentColor = ContextCompat.getColor(context, R.color.config_color_transparent);
            int transparentColor = Color.parseColor("#66000000");
            mTopBarSeparatorColor = transparentColor;
            mTopBarSeparatorHeight = 0;
            mTopBarBgColor = transparentColor;
        } else {
            init(context, null, R.attr.TopBarStyle);
        }
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTopBarSeparatorColor = array.getColor(R.styleable.TopBar_topbar_separator_color,Color.parseColor("#eeeeee"));
    ...
    void getCommonFieldFormTypedArray(Context context, TypedArray array) {
        mLeftBackDrawableRes = array.getResourceId(R.styleable.TopBar_topbar_left_back_drawable_id, R.mipmap.topbar_item_left_back);
        mTitleGravity = array.getInt(R.styleable.TopBar_topbar_title_gravity, Gravity.CENTER);
        mTitleTextSize = array.getDimensionPixelSize(R.styleable.TopBar_topbar_title_text_size, DisplayHelper.sp2px(context, 17));
        mTitleTextSizeWithSubTitle = array.getDimensionPixelSize(R.styleable.TopBar_topbar_title_text_size, DisplayHelper.sp2px(context, 16));
        mSubTitleTextSize = array.getDimensionPixelSize(R.styleable.TopBar_topbar_subtitle_text_size, DisplayHelper.sp2px(context, 11));
        mTitleTextColor = array.getColor(R.styleable.TopBar_topbar_title_color,Color.WHITE);
        mSubTitleTextColor = array.getColor(R.styleable.TopBar_topbar_subtitle_color,Color.WHITE);
        ...
        mTopBarImageBtnWidth = array.getDimensionPixelSize(R.styleable.TopBar_topbar_image_btn_width, DisplayHelper.dp2px(context, 48));
        mTopBarImageBtnHeight = array.getDimensionPixelSize(R.styleable.TopBar_topbar_image_btn_height, DisplayHelper.dp2px(context, 48));
        mTopBarTextBtnPaddingHor = array.getDimensionPixelSize(R.styleable.TopBar_topbar_text_btn_padding_horizontal, DisplayHelper.dp2px(context, 12));
        mTopBarTextBtnTextColor = array.getColorStateList(R.styleable.TopBar_topbar_text_btn_color_state_list);
        if (mTopBarTextBtnTextColor == null) {
            mTopBarTextBtnTextColor = getResources().getColorStateList(R.color.topbar_text_btn_color_state_list);
        }
        mTopBarTextBtnTextSize = array.getDimensionPixelSize(R.styleable.TopBar_topbar_text_btn_text_size, DisplayHelper.sp2px(context, 16));
    ...
    public TextView setTitle(String title) {
        ...
        if (LangHelper.isNullOrEmpty(title)) {
...
//            mTitleView = isEmoji ? new EmojiconTextView(getContext()) : new TextView(getContext());
    ...
    private void updateTitleViewStyle() {
        if (mTitleView != null) {
            if (mSubTitleView == null || LangHelper.isNullOrEmpty(mSubTitleView.getText())) {
    ...
    public void setSubTitle(String subTitle) {
        ...
        if (LangHelper.isNullOrEmpty(subTitle)) {
            titleView.setVisibility(GONE);
        } else {
            titleView.setVisibility(VISIBLE);
        }
    ...
    private TextView getSubTitleView() {
        if (mSubTitleView == null) {
            ...
            LinearLayout.LayoutParams titleLp = generateTitleViewAndSubTitleViewLp();
            titleLp.topMargin = DisplayHelper.dp2px(getContext(), 1);
    ...
    private LayoutParams generateTitleContainerViewLp() {
        return new LayoutParams(LayoutParams.MATCH_PARENT,DisplayHelper.dp2px(getContext(),48));
    }
    ...
    public AlphaImageButton addLeftBackImageButton() {
        return addLeftImageButton(mLeftBackDrawableRes, R.id.topbar_item_left_back);
    }
        ...
        if (mTitleContainerView != null) {
            ...
            if ((mTitleGravity & Gravity.HORIZONTAL_GRAVITY_MASK) == Gravity.CENTER_HORIZONTAL) {
                ...
                if (mLeftViewList.isEmpty()) {
                    //左侧没有按钮，标题离左侧间距
                    titleContainerViewLeft += ResHelper.getAttrDimen(getContext(),
