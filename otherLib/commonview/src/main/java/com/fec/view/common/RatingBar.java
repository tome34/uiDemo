...
public class RatingBar extends View{
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        this.starCount = mTypedArray.getInteger(R.styleable.RatingBar_starCountV2, 5);
        this.starEmptyDrawable = mTypedArray.getDrawable(R.styleable.RatingBar_starEmptyV2);
        this.starFillBitmap =  drawableToBitmap(mTypedArray.getDrawable(R.styleable.RatingBar_starFillV2));
        mTypedArray.recycle();
    ...
    public void setStarMark(float mark){
        if (integerMark) {
           // starMark = (int)Math.ceil(mark);
            int min = 3 ;
            int max = 7 ;
            float number = Math.round(mark * 10) * 1.0f / 10;
            try {
                String str = String.valueOf(number);
                String[] split = str.split("\\.");
                Integer int1 = Integer.valueOf(split[0]);
                Integer int2 = Integer.valueOf(split[1]);
                if (int2 < min ){
                   starMark = int1 * 1.0f ;
                }else if (int2 > max){
                    starMark = (int1 + 1) * 1.0f ;
                }else {
                    starMark = (int1 + 0.5f) * 1.0f ;
                }
            }catch (Exception e){
                e.printStackTrace();
                starMark = number;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0;i < starCount;i++) {
            ...
            Log.d("星星绘制1", starDistance+starSize+","+(starDistance + starSize) * i + starSize);
        }
        if (starMark > 1) {
